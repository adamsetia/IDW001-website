...
(function (factory) {
	...
	$.BigVideo = function(options) {
		...
		var defaults = {
			...
			container:$('body'),
			shrinkable:false,
			elemID: '#big-video-vid'
		};
		
		var BigVideo = {},
			player,
			vidEl = options['elemID'],
		...
		BigVideo.init = function() {
			if (!isInitialized) {
				...
				var elemName = settings.elemID.substr(1)
				var elemFlashAPI = settings.elemID+'_flash_api';
				$(elemFlashAPI)
					.attr('scale','noborder')
					.attr('width','100%')
					.attr('height','100%');
				...
				player.on('loadedmetadata', function(data) {
					if (document.getElementById(elemName+'_flash_api')) {
						// use flash callback to get mediaAspect ratio
						mediaAspect = document.getElementById(elemName+'_flash_api').vjs_getProperty('videoWidth')/document.getElementById(elemName+'_flash_api').vjs_getProperty('videoHeight');
					} else {
						// use html5 player to get mediaAspect
						mediaAspect = $(settings.elemID+'_html5_api').prop('videoWidth')/$(settings.elemID+'_html5_api').prop('videoHeight');
					}
					updateSize();
